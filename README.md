...
Let $ 1_{\Phi}(H, t) $ be an indicator function such that $ 1_{\Phi}(H, t) = 1 $ if the observer-pattern $ \Phi $ is instantiated at subjective time step $ t $ within history $ H $, and $ 0 $ otherwise.
...
*Note: While related to Kolmogorov Complexity $ K(H) $, the function $ C(H,t) $ specifically measures the cumulative work of the generative process relative to the observer's timeline.*
...
$$ P(H_t \mid 1_{\Phi} = 1) \propto 2^{-C(H, t)} $$
...
While the complexity $ K(H_{collatz}) $ is low, the complexity of an observer $ \Phi $ capable of existing within that chaos is extremely high. Using the chain rule of algorithmic complexity:
...
CAP predicts that the subjective "now" of an observer will be located at the earliest point in history where the cumulative complexity $ C(H,t) $ required to access a long-term, low-cost persistence path is minimized.
...
### 7.2 Cross-Observer Causal Interfaces (Narrative Shadows)
...
QI lacks a coherent measure of probability. CAP's $ 2^{-C} $ w
