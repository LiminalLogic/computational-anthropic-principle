...
Let $\mathbf{1}_\Phi(H, t)$ be an indicator function such that $\mathbf{1}_\Phi(H, t) = 1$ if the observer-pattern $\Phi$ is instantiated at subjective time step $t$ within history $H$, and $0$ otherwise.
...
$$ P(H_t \mid \mathbf{1}_\Phi = 1) \propto 2^{-C(H, t)} $$
